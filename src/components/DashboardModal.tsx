...
function CheckBox({ task, handleSelectionOfData, selectedData }: checkProps) {
  ...
  useEffect(() => {
    if (selectedData.length === 1 && selectedData[0].name !== "no name") {
      setFull(true);
    } else {
      setFull(false);
    }
...
function DashboardModal({
  ...
  const submitSelectedTasks = async (e: any) => {
    ...
    try {
      ...
      closeModal();
      toast.success("Your goal tasks are updated!");
    } catch (error) {
      console.log(error);
    ...
    >
      <div className="flex flex-col h-full gap-8 ">
        <div className="flex flex-row justify-between justify-self-start">
          <h2 className="font-semibold text-lg">Select your {type} taks</h2>
